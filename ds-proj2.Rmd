...
dat4 = read.csv("https://github.com/cforsythe/ViewPredictor/raw/master/GBvideos.csv")
dat$region=0
dat1$region=1
dat2$region=2
dat3$region=3
...
#Model Analysis
When looking at the models diagnostic plots the overall consensus is that the model follows patterns of a  model. However, there are a few interesting aspects in each graph. First, in the Residuals vs Fitted graph it can be seen that there is a slight parabola. This is a bit alarming as the pattern for a good linear relationship should be a be a horizontal line without a pattern. Second, in the Normal Q-Q graph it is shown that the residuals do a good job of following the line until the very end where it find its way well above the where it should be. This severe deviation is a sign of a bad model. Third, in the Scale-Location model 
...
##Model 2
```{r}
fit2 = lm(log(max_views) ~ log(max_comments) + log(max_likes) + log(max_dislikes) + region + days_trending , dat=tr_dat)
summary(fit2)
actuals = te_dat$max_views
